import marshallSignature from './marshall.js';
import constants from './constants.js';
import DBusBuffer from './dbus-buffer.js';

import headerSignature from './header-signature.json' with { type: 'json' };

export const unmarshalMessages = function messageParser(
  stream,
  onMessage,
  opts
) {
...
export function marshall(message) {
  ...
  if (message.signature && message.body) {
    bodyBuff = marshallSignature(message.signature, message.body);
  ...
  var headerBuff = marshallSignature('yyyyuu', header);
  var fields = [];
  ...
  var fieldsBuff = marshallSignature('a(yv)', [fields], 12);
