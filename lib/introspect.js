...
module.exports.introspectBus = async function(obj) {
  ...
  return module.exports.processXML(xml, obj);
};

module.exports.processXML = async function(xml, obj) {
  ...
  if (!result.interface) {
    if (result.node && result.node.length > 0 && result.node[0]['$']) {
      ...
      return module.exports.introspectBus(subObj);
    }
...
DBusInterface.prototype.addListener = DBusInterface.prototype.on = async function(
  signame,
  callback
) {
  ...
  if (!bus.signals.listeners(signalFullName).length) {
    ...
    // FIXME: What if it throws? - sonnyp
...
DBusInterface.prototype.removeListener = DBusInterface.prototype.off = async function(
  signame,
  callback
) {
  var bus = this.$parent.service.bus;
...
DBusInterface.prototype.$readProp = async function(propName) {
  var bus = this.$parent.service.bus;
